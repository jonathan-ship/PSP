...
if __name__ == "__main__":
    ...
    rl_model = ["1e-4_1_entire_0_1", "1e-4_1_entire_1_9", "1e-4_1_entire_2_8", "1e-4_1_entire_3_7", "1e-4_1_entire_4_6", "1e-4_1_entire_5_5", "1e-4_1_entire_6_4", "1e-4_1_entire_7_3", "1e-4_1_entire_8_2", "1e-4_1_entire_9_1", "1e-4_1_entire_1_0"]
    ...
    for num_block in num_block_list:
        sample_data = get_sample(num_block, cfg.tt_number)
        ...
        for test_i in sample_data.keys():
            ...
            for model in trained_model:
                ...
                print(
                    "{0} | {1} | {2} | Test {3} | Model = {4}".format(num_block, round(ddt, 4), round(pt_var, 4),
                                                                      test_i, model))
                ...
                if model not in ["SSPT", "ATCS", "MDD", "COVERT"]:
                    env = WeldingLine(num_block=num_block,
                                      ...
                                      ddt=ddt,
                                      is_train=False)
                    model_path = "./trained model/1_entire/{0}.pt".format(model)
        ...
        with open(simulation_dir + "/Test Tardiness({0}).json".format(num_block),
                  'w') as f:
            json.dump(output_tardiness, f)
        with open(simulation_dir + "/Test Setup({0}).json".format(num_block),
                  'w') as f:
