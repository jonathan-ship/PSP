...
class Process:
    ...
    def run(self):
        ...
        while True:
            ...
            self.planned_finish_time = self.env.now + part.length/part.avg_speed
            # 실제 작업 시간
...
class Routing:
    ...
    def run(self, location=None):
        if len(self.model["Source"].queue.items) > 0:
            block = np.random.choice(list(np.unique([job.block for job in self.model["Source"].queue.items])))
