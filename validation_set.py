...
if __name__ == "__main__":
    ...
    for e in range(episode, episode + num_episode + 1):
        ...
        while not done:
            ...
            step += 1
            available_action = copy.deepcopy(env.routing.waiting_jobs)
            action = q.sample_action(torch.from_numpy(state).float(), epsilon, available_action)
            env.routing.waiting_jobs.remove(action)
            # 환경과 연결
