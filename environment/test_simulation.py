...
class Routing:
    def __init__(self, env, model, block_info, monitor, routing=None):
        self.env = env
        self.model = model
    ...
    def run(self):
        while len(self.model["Source"].queue.items) > 0:
            ...
            if self.routing_rule == "RANDOM":
                self.routing_rule = np.random.choice(["SSPT", "MDD", "ATCS", "COVERT"])
            ...
            self.monitor.record(time=self.env.now, part=next_job.name, block=next_job.block,
                                event="Routing Finished", process=location)
    ...
    def ATCS(self):
        ...
        s_avg = np.mean(
            [self._get_setup_time(calling_line.job,
                                  self.block_info["Block_{0}".format(job_name.split("_")[1])][job_name])
             for job_name in job_list])
