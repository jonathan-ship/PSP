...
class WeldingLine:
        ...
        if calling_line.job is not None:
            ...
            def _cal_expected_finish_time(avg_pt, num_jobs):
                ...
                for _ in range(num_jobs):
                    if (expected_time + avg_pt) % 1440 <= 960:
                        ...
                        expected_time = (day + 1) * 1440 + avg_pt
            ...
            if len(non_setup_list) > 0:
                ...
                for non_setup_job in non_setup_list:
                    ...
                    finished_jobs = self.model["Sink"].finished[non_setup_job.block] if non_setup_job.block in \
                                                                                        self.model[
            ...
            if len(setup_list) > 0:
                ...
                for setup_job in setup_list:
                    ...
                    finished_jobs = self.model["Sink"].finished[setup_job.block] if setup_job.block in \
                                                                                        self.model[
                                                                                            "Sink"].finished.keys() else 0
    ...
    def _calculate_reward(self):
        ...
        for block_info in self.model["Sink"].finished_block:
            ...
            in_queue_time = self.model["Sink"].finished[block_name]["time"][1] - self.model["Sink"].finished[block_name]["time"][0]
            self.reward += - in_queue_time/1440
