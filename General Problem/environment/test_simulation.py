...
class Routing:
    def __init__(self, env, model, monitor, end_num, weight=None, routing_rule=None):
        self.env = env
        self.name = "Routing"
        self.model = model
        self.monitor = monitor
    ...
    def run(self):
        while self.created < self.end_num:
            ...
            routing_rule = self.routing_rule  # 정해진 routing_rule
            self.decision = None
    ...
    def _atcs(self):
        ...
        for job in job_list:
            first_term = np.exp(
                -np.max(job.due_date - job.processing_time - self.env.now, 0) / (k1 * p_avg)) if p_avg > 0 else 0
...
class Sink:
    ...
    def sink(self, job=None):
        self.completed += 1
        tardiness = np.max(self.env.now - job.due_date, 0)
